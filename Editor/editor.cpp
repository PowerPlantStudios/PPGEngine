...
{
  public:
    static void LoadScene(const std::filesystem::path &path_to_scene, Scene &ppge_scene,
...
void EditorWidget::OnAttach()
{
    ...
    auto &camera_component = camera_entity.AddComponent<CameraComponent>();
    {
        float fov = PI * 0.333f;
        float aspect_ratio =
            (float)DisplaySystem::Get().GetWidth() / (float)DisplaySystem::Get().GetHeight();
        camera_component.projection =
            Math::Matrix::CreatePerspectiveFieldOfView(fov, aspect_ratio, 0.1f, 1000.0f);
    }
    ...
    // Create lights
    {
        ...
        transform.rotation = Math::Quaternion::CreateFromYawPitchRoll(2.3561945, -0.7853982f, 0.0f);
        auto &dir_light = light.AddComponent<LightComponent>(LightComponent::LightType::DIRECTIONAL);
        dir_light.color = Math::Color(0.8f, 0.8f, 0.7f);
