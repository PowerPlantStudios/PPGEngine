...
namespace PPGE
{
	...
	inline std::ostream& operator<<(std::ostream& os, GamepadButtonCode gamepadButtonCode)
	{
		os << static_cast<int32_t>(gamepadButtonCode);
		return os;
	}

	inline std::ostream& operator<<(std::ostream& os, GamepadAxisCode gamepadAxisCode)
	{
		os << static_cast<int32_t>(gamepadAxisCode);
		return os;
	}
}
